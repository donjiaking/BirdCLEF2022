...
class MyDataset(Dataset):
    def __init__(self, df, mode='train', transforms=None):
        ...
        self.df = df.copy()
        if self.mode == 'train':
            self.df = self.df[self.df['rating'] >= CFG.min_rating]  # only preserve those reasonable data
            self.df["weight"] = self.df["rating"] / self.df["rating"].max()  # like 'credibility score'
